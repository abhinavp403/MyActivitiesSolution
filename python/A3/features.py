...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        hist,_=np.histogram(formants, bins=20, range=(0,5500))
        if self.debug:
            print("The distribution of formant frequencies gives us {} features..".format(len(hist)))
        return hist
    ...
    def _compute_pitch_features(self, window):
        ...
        hist,_=np.histogram(pitch_contour, bins=16, range=(0,128))
        hist = np.append(hist,np.mean(pitch_contour))
        hist = np.append(hist,np.std(pitch_contour))
        return hist
...
#        print(np.sum([mfcc_feats[n+i,:] - mfcc_feats[n-i,:] for i in range(1,n+1)],axis=0).shape)
        delta_coefficients = np.asarray([np.sum([(mfcc_feats[t+i,:] - mfcc_feats[t-i,:])*i for i in range(1,n+1)],axis=0) for t in range(n,n_frames-n)]).flatten()
        print(delta_coefficients.shape)
        if self.debug:
            print("There are {} delta coefficients".format(len(delta_coefficients)))
        return np.append(delta_coefficients, np.asarray(mfcc_feats).flatten())
