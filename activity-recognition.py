...
def onActivityDetected(activity):
    ...
    json_msg = ''
    if(activity == "Sitting"):
        json_msg = json.dumps({'user_id' : user_id, 'sensor_type' : 'SENSOR_SERVER_MESSAGE', 'message' : 'Sitting', 'data': activity}) + '\n'
    if(activity == "Walking"):
        json_msg = json.dumps({'user_id' : user_id, 'sensor_type' : 'SENSOR_SERVER_MESSAGE', 'message' : 'Walking', 'data': activity}) + '\n'
...
def predict(window):
    ...
    activity = classifier.predict(np.asarray(feature_vector).reshape((-1, 20)))
    ...
    onActivityDetected(class_names[int(activity[0])])    
